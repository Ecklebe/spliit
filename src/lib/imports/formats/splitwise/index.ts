...
class SplitwiseCsvFormat implements ImportFormat {
  ...
  async parseToInternal(content: string): Promise<{
    ...
    /**
     * Reconstruction works in index-based ids (`p0`, `p1`, ...) so that two
     * participants sharing a name stay distinct through the delta maths. The
     * import procedure, though, remaps by *display name* - see the
     * "adapters set participants to display names" comment in
     * groups/import/import-group.procedure.ts, which is also what the
     * Spliit-JSON adapter emits. So convert back to names at the boundary;
     * emitting `p0` here fails with "Invalid participant ID: p0".
     */
    const name = (id: string) => participantNameById[id] ?? id
    ...
    for (const row of parsed.rows) {
      ...
      for (const ge of parsedRow.groupExpenses) {
        expenses.push({
          ...
          notes: undefined,
          recurrenceRule: 'NONE',
          // Fork-only field (PR #172): a Splitwise export carries no location.
          location: null,
        })
      }

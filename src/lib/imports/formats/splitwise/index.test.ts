/**
 * Participants are asserted by display name, not by the adapter's internal
 * `p0`/`p1` indices: parseToInternal converts back to names at the boundary
 * because the import procedure remaps by name (see the note in index.ts).
 */
import fs from 'fs'
...
describe('SplitwiseCsvFormat', () => {
  ...
  it('handles zero-delta participant by excluding them from shares', () => {
    ...
    expect(expense!.paidFor.some((p) => p.participant === 'Charlie')).toBe(
      false,
    )
  })

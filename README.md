...
## Relationship to upstream Spliit

This is a fork of [spliit-app/spliit](https://github.com/spliit-app/spliit). It adds
optional accounts on top of Spliit, which is worth being explicit about, because
upstream has written up
[why Spliit doesn't ask you to create an account](https://spliit.app/blog/why-spliit-doesnt-ask-you-to-create-an-account).

That post is not an argument this fork disagrees with. It names both of the things
this fork is — self-hosting behind OIDC, and a community fork adding accounts under
the MIT licence — and the four invariants it actually defends are all preserved here:

- **A group link stays the capability.** Anyone with the link can view and edit,
  signed in or not. No procedure under `src/trpc/routers/groups/**` requires
  authentication; auth guards only `src/trpc/routers/sync/**` and
  `src/trpc/routers/admin/**`. A synced group is a bookmark, never an ACL.
- **Participants stay labels, not identities.** `Participant` has no relation to
  `User`. Picking "who am I" in a group is still a per-device choice about a
  user-chosen name.
- **No user database in upstream's sense.** No passwords are stored. A `User` row
  holds only what your own OIDC provider returns, on your own instance.
- **No account is still the default.** With `OIDC_PROVIDERS` unset — the default —
  login is not merely unlinked but absent: no nav entry, no sign-in UI, no sync, and
  `/api/auth/*` 404s. A build with no provider configured behaves exactly like
  upstream.

The one place this fork does go further is the admin-configurable **require login to
create groups** setting (see [Instance settings](#instance-settings) below). It
defaults to off, is ignored entirely unless an OIDC provider is configured, and gates
*creation only* — it never gates viewing or joining a group that already exists. It
exists so a self-hoster can stop their instance being a free public group host, which
is the same escape hatch the post points at, expressed in the app rather than in a
reverse proxy.

### Instance settings

When `ENABLE_ADMIN` is on and you hold the `admins` role, `/admin` exposes runtime
settings stored in the database (`InstanceSettings`), changeable without a redeploy:

- **Require login to create groups** (default: off) — when on, `groups.create` and
  `groups.importFromFile` both reject anonymous callers, and the create entry points
  are hidden. Existing groups stay open to anyone with the link.
